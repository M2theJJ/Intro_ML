...
class DirectoryIterator(Iterator):
    ...
    def __init__(self, directory, image_data_generator,triplet_path,
        ...
        for dirpath in (os.path.join(directory, subdir) for subdir in classes):
            results.append(pool.apply_async(_list_valid_filenames_in_directory,(dirpath, white_list_formats,
    ...
    def next(self):
        ...
        for i, j in enumerate(index_array):
            fname = "SimilarityClass1\\"+(self.filenames[j]+".jpg")[3:]
...
class DataGenerator(object):
    ...
    def get_train_generator(self, batch_size):
        return self.idg.flow_from_directory("food_resized",
                                            batch_size=batch_size,
                                            target_size=self.target_size,shuffle=False,
                                            triplet_path  ='train_tripletsBis.txt'
                                           )
...
deep_rank_model.compile(loss=_loss_tensor, optimizer=SGD(lr=0.001, momentum=0.9, nesterov=True), run_eagerly=False)


train_steps_per_epoch = int((59496)/batch_size)
train_epocs = 25
